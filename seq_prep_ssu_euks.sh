...
for library in $(sed '1d' sample_tag_primer_list.txt | cut -f6 | sort | uniq)
do
  sed '1d' sample_tag_primer_list.txt | awk -v FS="\t" -v OFS="\t" -v library=${library} '$6 == library' > "Library_"${library}"_index_primer.txt"
done
