...
# load packages
require(dada2) # did not work, so using the code bellow to install dada2 first
...
#load("dada2_ssu_pros.Rdata")

# specify path to input fastq files
path <- "Clipped" # does not work because I dont have this fastq files after demux on GitBash. too heavy??
...
filt.out.fr <- filterAndTrim(
  ...
  rev = fnRs.fr, 
  filt.rev = filtRs.fr,
  truncLen = c(260, 200), # can be adapted
  maxN = 0,
  minQ = 2,
  maxEE = c(3, 3), # can be adapted
  truncQ = 0, 
  rm.phix = TRUE,
  compress = F,
  multithread = 200
...
mergers.fr <- mergePairs(
  ...
  dadaRs.fr, 
  filtRs.fr, 
  minOverlap = 10, #default is 12 bases overlap, here we choose more stringent setting
  verbose = TRUE
)
...
# doing the next steps in another script now...("dada2_ssu_all_pros.Rdata")
